...
__global__ void bounce_back(float * fOut_new,float * fOut, float * fIn,
			    int * snl, 
  ...
  if(tid<nnodes){
    if(snl[tid]==1){
      ...
      fOut_new[spd*nnodes+tid]=fIn[spd_src*nnodes+tid];
    }else{
      fOut_new[spd*nnodes+tid]=fOut[spd*nnodes+tid];
    }
  }
...
err_t jktFunction(int nlhs,mxArray * plhs[], int nrhs, mxArray * prhs[]){
  ...
  if(nrhs!=4)
    return err("Usage: fOut = bounce_back_jkt(fOut,fIn,snl,bb_spd)");
  ...
  bounce_back<<<GRIDS,BLOCKS>>>(fOut_new_d,fOut_d,
