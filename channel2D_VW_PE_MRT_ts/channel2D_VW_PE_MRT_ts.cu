...
__global__ void channel2D_VW_PE_MRT_ts(float * fOut, float * fIn, int * inl, 
				      int * onl, int * snl, float * ux_p, 
				      float rho_out, float * omega_op,int Nx, int Ny){
...
err_t jktFunction(int nlhs, mxArray * plhs[], int nrhs, mxArray * prhs[]){
  ...
  if(nrhs!=10)
    return err("Usage: channel2D_VW_PE_MRT_ts(fOut,fIn,inl,onl,snl,ux_p,rho_out,omega_op,Nx,Ny");
