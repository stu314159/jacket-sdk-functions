//uniform velocity inlet, uniform pressure outlet, D2Q9 lattice
//solid object may be added optionally.
#include "jacketSDK.h"
#include <cuda.h>
...
__global__ void time_step(float * fOut, float * fIn, int * inl,int *onl,
			  int * snl, float omega, int Nx,int Ny, float u_bc,
			  float rho_out, float * u_p, float * rho_p){
...
err_t jktFunction(int nlhs,mxArray * plhs[],int nrhs,mxArray * prhs[]){
  ...
  if(nrhs!=12)
    return err("Usage: channelFlow2D_ts(fOut,fIn,inl,onl,snl,omega,Nx,Ny,u_bc,rho_out,u_p,rho_p");
  ...
  float u_bc = mxGetScalar(prhs[8]);
  float rho_out = mxGetScalar(prhs[9]);
  mxArray * m_u_p = prhs[10];
  mxArray * m_rho_p = prhs[11];
  ...
  jkt_mem((void**)&u_p,m_u_p);
  jkt_mem((void**)&rho_p,m_rho_p);

  dim3 BLOCKS(TPB,1,1);
  dim3 GRIDS((Nx*Ny+TPB-1)/TPB,1,1);

  time_step<<<GRIDS,BLOCKS>>>(fOut,fIn,inl,onl,snl,omega,Nx,Ny,u_bc,
			      rho_out,u_p,rho_p);
