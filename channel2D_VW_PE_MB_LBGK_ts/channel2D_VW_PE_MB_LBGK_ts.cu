...
__global__ void channel2D_VW_PE_MB_LBGK_ts(float * fOut, float * fIn,
					   int * inl, 
					   int * onl, int * snl, int * mbnl,
					   float * ux_p, float * uy_p,
					   float rho_out, float nu,
					   int Nx, int Ny){
...
err_t jktFunction(int nlhs, mxArray * plhs[], int nrhs, mxArray * prhs[]){
  ...
  if(nrhs!=12)
    return err("Usage: channel2D_VW_PE_MB_LBGK_ts(fOut,fIn,inl,onl,snl,mbnl,ux_p,uy_p,rho_out,nu,Nx,Ny");
  ...
  channel2D_VW_PE_MB_LBGK_ts<<<GRIDS,BLOCKS>>>(fOut,fIn,inl,
