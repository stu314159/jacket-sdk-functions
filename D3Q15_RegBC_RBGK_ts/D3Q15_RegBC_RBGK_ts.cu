...
__global__ void D3Q15_RegBC_RBGK_ts(const float * fIn, float * fOut,
				    const int * SNL,
				    ...
				    const int Nx, const int Ny, const int Nz)
{
  ...
  if(tid<nnodes){
	...
	f14=fe14+(f7-fe7); 
      ...
      //now, multiply by f# = ((ft#)*Pi_flat)*Q_flat'
...
err_t jktFunction(int nlhs,mxArray * plhs[],int nrhs,mxArray * prhs[]){
  ...
  if(nrhs!=11)
    return err("Usage:D3Q15_RegBC_RBGK_ts(fIn,fOut,SNL,VW_nl,VW_uz,PE_nl,rho_out,omega,Nx,Ny,Nz)");
  ...
  D3Q15_RegBC_RBGK_ts<<<GRIDS,BLOCKS>>>(fIn,fOut,SNL,VW_nl,VW_uz,PE_nl,rho_out,
					omega,Nx,Ny,Nz);
