...
__global__ void D3Q19_RegVwPe_SNL_MRT_ts(const float * fIn, float * fOut,
				    const int * SNL,
...
err_t jktFunction(int nlhs,mxArray * plhs[],int nrhs,mxArray * prhs[]){
  ...
  if(nrhs!=11)
    return err("Usage:D3Q19_RegVwPe_SNL_MRT_ts(fIn,fOut,SNL,VW_nl,VW_uz,PE_nl,rho_out,M,Nx,Ny,Nz)");
  ...
  D3Q19_RegVwPe_SNL_MRT_ts<<<GRIDS,BLOCKS>>>(fIn,fOut,SNL,VW_nl,VW_uz,PE_nl,rho_out,
					M,Nx,Ny,Nz);
