...
err_t jktFunction(int nlhs,mxArray * plhs[],int nrhs,mxArray * prhs[]){
  ...
  if(nrhs!=18)
    return err("Usage:D3Q19_RegVwPe_SNL_VNL_MRT_2NodeList_ts(fIn,fOut,SNL,VW_nl,PE_nl,rho_out,VNL,ux_p,uy_p,uz_p,M,Nx,Ny,Nz,NodeList1,numNL1,NodeList2,numNL2)");
  ...
  mxArray * m_NodeList1=prhs[14];
  int numNL1 = mxGetScalar(prhs[15]);
  mxArray * m_NodeList2=prhs[16];
  int numNL2=mxGetScalar(prhs[17]);
  ...
  jkt_mem((void**)&M,m_M);
  jkt_mem((void**)&NodeList1,m_NodeList1);
  jkt_mem((void**)&NodeList2,m_NodeList2);

  dim3 BLOCKS(TPB,1,1);
  dim3 GRIDS((numNL1+TPB-1)/TPB,1,1);
  ...
  dim3 BLOCKS2(TPB,1,1);
  dim3 GRIDS2((numNL2+TPB-1)/TPB,1,1);

  D3Q19_RegVwPe_SNL_VNL_MRT_NodeList_ts<<<GRIDS2,BLOCKS2>>>(fIn,fOut,SNL,VW_nl,PE_nl,rho_out,
							  VNL, ux_p,uy_p,uz_p,
							  M,Nx,Ny,Nz,
							  NodeList2,numNL2);
