...
__global__ void channel3D_D3Q15_LBGK_ts(float * fOut,  float * fIn,int * snl,
					int * inl, int * onl, float * uz_bc_in,
					float * uz_bc_out, const float omega,
  ...
  if((X<Nx)&&(Y<Ny)&&(Z<Nz)){
    ...
    float uz_in; float uz_out;


    if(onl[tid]==1){
      uz_out = uz_bc_out[tid];
...
err_t jktFunction(int nlhs,mxArray * plhs[], int nrhs, mxArray * prhs[]){
  ...
  channel3D_D3Q15_LBGK_ts<<<GRIDS,BLOCKS>>>(fOut_d,fIn_d,snl_d,inl_d,onl_d,
					    uz_bc_in,uz_bc_out,omega,Nx,Ny,Nz);
