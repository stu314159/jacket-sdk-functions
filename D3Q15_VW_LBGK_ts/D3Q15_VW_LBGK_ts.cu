//uses Regularized Boundary conditions for Velocity on a West boundary.
...
__global__ void D3Q15_VW_LBGK_ts(const float * fIn, float * fOut,
  ...
  if(tid_nd<numIN){
    ...
    //get non-equilibrium parts for strain tensor..
    ft1=f1-fe1; 
    ft2=f2-fe2;
    ft3=f3-fe3;
    ft4=f4-fe4;
    ...
    //now stream in all directions except to the west (negative Z direction)
...
err_t jktFunction(int nlhs,mxArray * plhs[],int nrhs,mxArray * prhs[]){
  ...
  if(nrhs!=9)
    return err("Usage: D3Q15_VW_LBGK_ts(fIn,fOut,VW_nl,VW_uz,numIN,omega,Nx,Ny,Nz");

  mxArray * m_fIn = prhs[0];
  mxArray * m_fOut = prhs[1];
  mxArray * m_VW_nl = prhs[2];
  mxArray * m_VW_uz = prhs[3];
  ...
  D3Q15_VW_LBGK_ts<<<GRIDS,BLOCKS>>>(fIn,fOut,VW_nl,VW_uz,numIN,omega,Nx,Ny,Nz);
