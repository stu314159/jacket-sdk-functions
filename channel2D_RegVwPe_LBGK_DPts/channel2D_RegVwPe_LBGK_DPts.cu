...
#define TPB 96
__global__ void channel2D_RegVwPe_LBGK_ts(double* fOut, double* fIn, int * inl, 
				      int * onl, int * snl, double* ux_p, 
				      double rho_out, double nu,int Nx, int Ny){
...
err_t jktFunction(int nlhs, mxArray * plhs[], int nrhs, mxArray * prhs[]){
  ...
  if(nrhs!=10)
    return err("Usage: channel2D_RegVwPe_LBGK_ts(fOut,fIn,inl,onl,snl,ux_p,rho_out,nu,Nx,Ny");
  ...
  channel2D_RegVwPe_LBGK_ts<<<GRIDS,BLOCKS>>>(fOut,fIn,inl,onl,snl,ux_p,rho_out,nu,Nx,Ny);
