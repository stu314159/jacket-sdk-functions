...
__global__ void pc_D3Q15_VW_Reg(float * fIn, float * fEq, 
				float * rho_d, float * ux_d,
				float * uy_d, float * uz_d,
				int * VW_nl, float * VW_uz,
				const int numIN,const int nnodes){
  ...
  if(tid_nd<numIN){
    ...
    //rho = 2.0*(f6+f11+f12+f13+f14)+(f0+f1+f2+f3+f4);
...
err_t jktFunction(int nlhs,mxArray * plhs[],int nrhs,mxArray * prhs[]){
  ...
  if(nrhs!=10)
    return err("Usage: pc_D3Q15_VW_Reg(fIn,fEq,rho,ux,uy,uz,VW_nl,VW_uz,numIN,nnodes)");
  ...
  pc_D3Q15_VW_Reg<<<GRIDS,BLOCKS>>>(fIn,fEq,rho,ux,uy,uz,VW_nl,VW_uz,numIN,nnodes);
