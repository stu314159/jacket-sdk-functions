...
err_t jktFunction(int nlhs, mxArray * plhs[], int nrhs, mxArray * prhs[]){
  ...
  if(nrhs!=9)
    return err("Usage: D2Q9_SNL_VNL_MRT_ts(fOut,fIn,snl,vnl,ux_p,uy_p,omega_op,Nx,Ny");

  mxArray * m_fOut = prhs[0];
  mxArray * m_fIn = prhs[1];
  mxArray * m_snl = prhs[2];
  mxArray * m_vnl = prhs[3];
  mxArray * m_ux_p = prhs[4];
  mxArray * m_uy_p = prhs[5];
  mxArray *  m_omega_op = prhs[6];
  int Nx = mxGetScalar(prhs[7]);
  int Ny = mxGetScalar(prhs[8]);

  int nnodes = Nx*Ny;

  float * fOut; float * fIn; int * snl; int * vnl;
  float * ux_p; float * uy_p; float * omega_op;
  ...
  D2Q9_SNL_VNL_MRT_ts<<<GRIDS,BLOCKS>>>(fOut,fIn,snl,vnl,ux_p,uy_p,omega_op,Nx,Ny);
