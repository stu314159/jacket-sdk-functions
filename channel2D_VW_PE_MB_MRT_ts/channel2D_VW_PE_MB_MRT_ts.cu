...
__global__ void channel2D_VW_PE_MB_MRT_ts(float * fOut, float * fIn,
					   int * inl, 
					   int * onl, int * snl, int * mbnl,
					   float * ux_p, float * uy_p,
					   float rho_out, float * omega_op,
					   int Nx, int Ny){
...
err_t jktFunction(int nlhs, mxArray * plhs[], int nrhs, mxArray * prhs[]){
  ...
  if(nrhs!=12)
    return err("Usage: channel2D_VW_PE_MB_MRT_ts(fOut,fIn,inl,onl,snl,mbnl,ux_p,uy_p,rho_out,omega_op,Nx,Ny");
  ...
  mxArray * m_mbnl=prhs[5];
  mxArray * m_ux_p = prhs[6];
  mxArray * m_uy_p = prhs[7];
  float rho_out = mxGetScalar(prhs[8]);
  mxArray * m_omega_op = prhs[9];
  int Nx = mxGetScalar(prhs[10]);
  int Ny = mxGetScalar(prhs[11]);

  int nnodes = Nx*Ny;

  float * fOut; float * fIn; int * inl; int * onl; int * snl; int * mbnl;
  float * ux_p; float * uy_p;
  float * omega_op;
  ...
  channel2D_VW_PE_MB_MRT_ts<<<GRIDS,BLOCKS>>>(fOut,fIn,inl,
					       onl,snl,mbnl,ux_p,uy_p,
					       rho_out,omega_op,Nx,Ny);
