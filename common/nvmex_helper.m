...
elseif ispc
    ...
    quote = '"'; % quote file paths to handle spaces
    ...
    for i=1:numel(varargin)
        if any(varargin{i} == ' ')
            varargin{i} = [ quote varargin{i} quote ];
        end
    end

    % Format the mex command
    cmdargs = ['-called_from_matlab -matlab ' quote matlabroot quote ' ' sprintf(' %s', varargin{:})];


    % determine nvmex.pl path
    s = dbstack('-completenames');
    [pathstr script_name] = fileparts(s(1).file);
    nvmex_path = [quote pathstr '\nvmex.pl' quote];

    cmdtool = [quote matlabroot '\sys\perl\win32\bin\perl.exe' quote ' ' ...
               '-I' quote matlabroot '\bin' quote ' ' nvmex_path];
