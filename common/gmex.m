function varargout = gmex(varargin)
%MEX   Compile MEX-function
%
%   Usage:
%       gmex [options ...] file [files ...]
%
%   Description:
...
% arch
if (strcmp(computer, 'PCWIN64')), arch = 64; else, arch = 32; end

% nvmexopts path
s = dbstack('-completenames');
[pathstr, script_name] = fileparts(s(1).file);
opts_path = [pathstr '\nvmexopts' num2str(arch) '.bat'];

% arch dep library selection, lib path select
libjacket = '-ljacketSDK -ljacketDLA -laf';

% cuda paths (assume that this is correct across archs)
cuda_lib_path = getenv('CUDA_LIB_PATH');
if numel(cuda_lib_path) == 0
  s = sprintf(['CUDA_LIB_PATH environment variable is not defined. Is the CUDA ' ...
               'toolkit properly installed?' ...
               '\nFor more information on this error, read the FAQ entry for ' ...
               'Error 127: \nhttp://accelereyes.com/faq?q=127\n']);
  error(s);
end
cuda_lib_path = [ '-L' cuda_lib_path ];

cuda_inc_path = getenv('CUDA_INC_PATH');
if numel(cuda_inc_path) == 0
  s = sprintf(['CUDA_INC_PATH environment variable is not defined. Is the CUDA ' ...
               'toolkit properly installed?' ...
               '\nFor more information on this error, read the FAQ entry for ' ...
               'Error 127: \nhttp://accelereyes.com/faq?q=127\n']);
  error(s);
end
cuda_inc_path = [ '-I' cuda_inc_path ];


% library, include search path (same place as gpu_entry)
s = which('gpu_entry');
if isempty(s)
  s = sprintf(['gpu_entry not found on the path. Did you add <JACKET_PATH>/engine to the path? ' ...
               '\nFor more information on this error, read the FAQ entry for ' ...
               'Error 126: \nhttp://accelereyes.com/faq?q=126\n']);
  error(s);
end
[pathstr, script_name] = fileparts(s);
jacket_lib_path = ['-L' pathstr];
jacket_inc_path = ['-I' pathstr '\include'];

% all arguments for compile
args = {'-f' opts_path ...
        cuda_lib_path cuda_inc_path jacket_lib_path ...
        '-lcuda' '-lcudart' '-lcusparse' ...
        '-ljacketSDK'  '-ljacketDLA' '-laf', ...
        jacket_inc_path '-largeArrayDims'};
args = {args{:} varargin{:}};

try
    verify_lib_settings();
    [varargout{1:nargout}]=nvmex_helper(args{:});
    fprintf('\nBuild completed\n');
    if arch == 64
      fprintf('Please make sure the DLLs in <JACKET_PATH>/engine/bin64 are available on the environment variable PATH when deployed\n');
    else
      fprintf('Please make sure the DLLs in <JACKET_PATH>/engine/bin64 are available on the environment variable PATH when deployed\n');
    end
catch
...
function verify_lib_settings()
  % Silently load DLLs required and report any errors
    gzeros(1);
end
